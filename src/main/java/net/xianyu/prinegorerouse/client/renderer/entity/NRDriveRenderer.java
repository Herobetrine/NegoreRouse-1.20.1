...
public class NRDriveRenderer<T extends EntityNRDrive> extends EntityRenderer<T> {
    ...
    public void render(T entity, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource bufferSource, int packedLightIn) {
        try (MSAutoCloser msac = MSAutoCloser.pushMatrix(matrixStack)) {
            ...
            baseAlpha = Math.max(0.0D, -Math.pow(baseAlpha - 1.0D, 4.0D) + 0.75D);
            
            matrixStack.mulPose(Axis.YP.rotationDegrees(
                    Mth.rotLerp(partialTicks, entity.yRotO, entity.getYRot()) - 90.0F));
            ...
            int alpha = (0xFF & (int)(255.0D * baseAlpha)) << 24;
            ...
            BladeRenderState.renderOverridedLuminous(
                    ItemStack.EMPTY, model, "drive_5ye", TEXTURE, matrixStack, bufferSource, packedLightIn);
        }
    }
}